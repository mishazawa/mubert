...
CoatOutput coat_pattern(in DisplacePatternOutput data, float animation) {
  ...
  vec3 viewDir = normalize(vCameraPositionW); // or -normalize(fragPos) in view space
  vec3 reflected = reflect(-vCameraPositionW, normalize(data.normal));
  float eta = 1.0 / 1.5;                   // air to glass
  vec3 refracted = refract(vCameraPositionW, normalize(data.normal), eta);
