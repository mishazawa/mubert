...
vec3 displace_ (in vec3 P, in vec3 N, in vec3 patt, in float animation) {
  ...
  for (int j = 0; j < 1; j++) {
    ...
    vec3 noise2 = vec3(
        snoise(npos2 + 11.1 + vec3(0.0, 0.0, PHASE)),
  ...
  newPosition = P*0.0+patt*8.0* DISPLACE_SCALE;
  newPosition = P + vec3(
...
DisplacePatternOutput displace_pattern(in DisplacePatternInput data, float animation) {
  ...
  vec3 n = calcNormalFromSamples(
    p,
    displace_(samples.a, data.normal, patt, animation),
...
CoatOutput coat_pattern(in DisplacePatternOutput data, float animation) {
  ...
  float emission = snoise(data.pattern - 4.4 + vec3(0.0, 0.0, uSeed*20.0))*0.5+0.5;
  emission = gain(emission, 4.0);
  float iridescence = snoise(data.pattern - 5.4 + vec3(0.0, 0.0, uSeed*30.0))*0.5+0.5;
  ...
  vec3 newPosition = data.position*0.0*random(uSeed)*1.0+data.pattern*8.0* DISPLACE_SCALE;
  newPosition = vec3(
    snoise(newPosition + vec3(0.5, 0.0, 0.0)),
    snoise(newPosition + vec3(10.5, 0.0, 0.0)),
    snoise(newPosition + vec3(20.5, 0.0, 0.0))
  ...
  newColor = mix(
    mix(uColor1, uColor2, gain(newPosition.x, 4.0)),
